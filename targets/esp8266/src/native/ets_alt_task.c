...
ICACHE_RAM_ATTR static inline int prio2id(uint8_t prio) {
    int id = PRIO2ID(prio);
...
ICACHE_RAM_ATTR bool ets_task(os_task_t task, uint8 prio, os_event_t *queue, uint8 qlen) {
    static unsigned cnt;
...
ICACHE_RAM_ATTR bool ets_post(uint8 prio, os_signal_t sig, os_param_t param) {
...
ICACHE_RAM_ATTR bool ets_loop_iter(void) {
...
#if SDK_BELOW_1_1_1
ICACHE_RAM_ATTR void my_timer_isr(void *arg) {
//    uart0_write_char('+');
    ets_post(0x1f, 0, 0);
}
...
ICACHE_RAM_ATTR void ets_timer_init() {
...
ICACHE_RAM_ATTR bool ets_run(void) {
#if USE_ETS_TASK
    #if SDK_BELOW_1_1_1
    ets_isr_attach(10, my_timer_isr, NULL);
    #endif
...
ICACHE_RAM_ATTR void ets_set_idle_cb(void (*handler)(void *), void *arg) {
