...
/* We use this macro for the following reasons:
 * 1) esp8266_Wifi_XXX macro is defined only when the class of the field is not eliminated in dead-code elimination process.
 * 2) Unused code in irom0.* section are (currently) linked, even if --gc-section flag is used. 
 */
...
DEFINE(jbyte, get_1opmode, (), \
	return (int8_t)wifi_get_opmode(); \
)

DEFINE(jbyte, get_1opmode_1default, (), \
	return (int8_t)wifi_get_opmode_default(); \
)

DEFINE(jboolean, set_1opmode, (jbyte mode),  \
	return wifi_set_opmode(mode); \
)

DEFINE(jboolean, set_1opmode_1current, (jbyte mode), \
...
DEFINE(jboolean, station_1connect, (),  \
	return wifi_station_connect(); \
)

DEFINE(jboolean,  station_1disconnect, (), \
	return wifi_station_disconnect(); \
)

DEFINE(jbyte, station_1get_1connect_1status, (), \
...
DEFINE(jboolean, station_1ap_1number_1set, (int8_t ap_number), \
...
DEFINE(jbyte, station_1ap_1change, (int8_t ap_id),  \
	return wifi_station_ap_change(ap_id); \
)

DEFINE(jbyte, station_1get_1current_1ap_1id, (), \
	return wifi_station_get_current_ap_id(); \
)

DEFINE(jbyte, station_1get_1auto_1connect, (), \
	return wifi_station_get_auto_connect(); \
)

DEFINE(jboolean, station_1set_1auto_1connect, (int8_t b), \
	return wifi_station_set_auto_connect(b); \
)

DEFINE(jboolean, station_1dhcpc_1start, (), \
	return wifi_station_dhcpc_start(); \
)

DEFINE(jboolean, station_1dhcpc_1stop, (), \
	return wifi_station_dhcpc_stop(); \
)

DEFINE(int, station_1dhcpc_1status, (), \
	return (int)wifi_station_dhcpc_status();\
)

DEFINE(jboolean, station_1dhcpc_1set_1maxtry, (int max), \
	return wifi_station_dhcpc_set_maxtry(max); \
)

DEFINE(jboolean, station_1set_1reconnect_1policy, (int b), \
	return wifi_station_set_reconnect_policy(b);	 \
)

DEFINE(int, station_1get_1rssi, (), \
...
DEFINE(int, station_1get_1hostname, (void* name), \
...
DEFINE(jbyte, softap_1get_1station_1num, (), \
...
DEFINE(jboolean,  softap_1dhcps_1stop, (), \
...
DEFINE(jboolean, softap_1set_1dhcps_1lease_1time, (int minute), \
	return wifi_softap_set_dhcps_lease_time(minute); \
)

DEFINE(int,  softap_1get_1dhcps_1lease_1time, (), \
	return wifi_softap_get_dhcps_lease_time(); \
)

DEFINE(int, softap_1dhcps_1status, (), \
	return (int)wifi_softap_dhcps_status(); \
)

DEFINE(jboolean, softap_1set_1dhcps_1offer_1option, (jbyte level, int optarg), \
	jbyte arg = optarg; \
	return wifi_softap_set_dhcps_offer_option(level, &arg); \
)

DEFINE(jboolean, set_1phy_1mode, (int mode), \
	return wifi_set_phy_mode(mode); \
)

DEFINE(int, get_1phy_1mode, (), \
...
DEFINE(jboolean, set_1macaddr, (int if_index, void* macaddr), \
	return wifi_set_macaddr(if_index, macaddr); \
)

DEFINE(jboolean, get_1macaddr, (int if_index, void* macaddr), \
	return wifi_get_macaddr(if_index, macaddr); \
)

DEFINE(jboolean, set_1sleep_1type, (int type),  \
	return wifi_set_sleep_type(type);	 \
)

DEFINE(int, get_1sleep_1type, (), \
	return (int)wifi_get_sleep_type(); \
)

DEFINE(void, status_1led_1install, (int gpio_id, int gpio_name, int gpio_func), \
	wifi_status_led_install(gpio_id, gpio_name, gpio_func);	 \
)

DEFINE(void, status_1led_1uninstall, (), \
	wifi_status_led_uninstall();	 \
)

DEFINE(jboolean, set_1broadcast_1if, (int intf), \
	return wifi_set_broadcast_if(intf); \
)

DEFINE(int, get_1broadcast_1if, (), \
	return wifi_get_broadcast_if(); \
)

DEFINE(jboolean, wps_1enable, (int wps_type), \
	return wifi_wps_enable(wps_type); \
)

DEFINE(jboolean, wps_1disable, (), \
	return wifi_wps_disable(); \
)

DEFINE(jboolean, wps_1start, (), \
	return wifi_wps_start(); \
)

DEFINE(int, send_1pkt_1freedom, (void* buf, int len, int sys_seq), \
	return wifi_send_pkt_freedom(buf, len, sys_seq);	 \
)

DEFINE(int, rfid_1locp_1recv_1open, (), \
	return wifi_rfid_locp_recv_open(); \
)

DEFINE(void, rfid_1locp_1recv_1close, (), \
	wifi_rfid_locp_recv_close();	 \
)

DEFINE(void, enable_1gpio_1wakeup, (int i, int intr_status), \
/**/
)

DEFINE(void, disable_1gpio_1wakeup, (),	\
/**/
)
...
DEFINE(void, dispose_1scan_1done_1event, (int rawData), \
