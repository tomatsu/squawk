 ...
 * NetUtil
 */
int Java_esp8266_NetUtil_resolve(char* host) {
	ip_addr_t addr;
 ...
 * UDPEndpoint
 */
int Java_com_sun_squawk_io_UDPEndpoint_create0() {
}

int Java_com_sun_squawk_io_UDPEndpoint_connect0(int handle, int addr, int port) {
}

int Java_com_sun_squawk_io_UDPEndpoint_close0(int handle) {
}

int Java_com_sun_squawk_io_UDPEndpoint_send0(int handle, uint8_t* buf, int off, int len) {
	udp_context_t* udp = (udp_context_t*)handle;
	return squawk_udp_send(udp, &udp->_pcb->remote_ip, udp->_pcb->remote_port, buf + off, len);
}

int Java_com_sun_squawk_io_UDPEndpoint_send1(int handle, int addr, int port, uint8_t* buf, int off, int len) {
	udp_context_t* udp = (udp_context_t*)handle;
	return squawk_udp_send(udp, addr, port, buf + off, len);
}

int Java_com_sun_squawk_io_UDPEndpoint_receive0(int handle, uint8_t* buf, int off, int len) {
